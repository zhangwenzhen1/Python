...
def processor():
    ...
    for key in data_dict.keys():
        ...
        pci1 = int(arr[pci_index])

        #小区编号先自加
        cell_id += 1

        #将此行数据标记为已处理
        data_dict[key] = key.replace("\n", "") + "," + str(cell_id) + "," + str(pci1 % 3)
        ...
        for line in data_dict.keys():
            ...
            pci2 = int(jrr[pci_index])

            #距离小于50米，则标记为已处理，同时写出到文件中
            if (lon2 - lon1)*(lon2 - lon1) + (lat2 - lat1)*(lat2 - lat1) < 0.01:
                    data_dict[line] = line.replace("\n", "") + "," + str(cell_id)  + "," + str(pci2 % 3)
