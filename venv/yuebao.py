...
zb=pd.merge(rl,gc,on='小区ID',how='left',suffixes=('', '_y')) # pandas csv表左连接
              # 删除不用的列
zb.drop( axis =1, columns = ['省份', '主设备厂家','室内外基站属性','天线挂高', 'CQI平均值', '地市_y','小区名称_y',
                             '是否属于五期扩容工程','是否多层网小区', '基础频点小区', '对应的基础频点小区ID',
                            '设备型号(仅室外微站填写)','区域类型','通道最大发射功率(W)','分QCI的建立成功次数(QCI5)',
                             '小区用户面上行字节数（QCI5）', '小区用户面下行字节数（QCI5）' ,'下行PDSCH PRB占用平均数',
                             ...
                             '上行PDSCH PRB占用数', '上行PDSCH PRB可用数', '下行用户平均速率', '上行用户平均速率',
                             '平均E-RAB数', 'E-RAB建立成功数','VOLTE语音话务量', 'VOLTE语音峰值用户数',
                             '下行VOLTE占用PRB总数', '上行VOLTE占用PRB总数', 'E-RAB拥塞率(无线资源不足)','无线掉线率(剔除UI)'], inplace=True)
# print(zb.columns)
              #删除重复行
...
# print(z_mean)
# zb1.to_csv('D:\guangdong\meanluliang1.csv',header=1,encoding='gbk') #保存列名存储
                     ####计算流量系数
...
zb.to_csv('D:\guangdong\liuliangxishu0727-0802.csv',header=1,encoding='gbk')
    ...
    #地市业务均衡统计
# g1 = zb.loc[(zb['流量系数'].isnull()) , ['地市','小区ID','流量系数']]
